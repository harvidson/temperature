...
// //this one will go away; use more tailored version below (/:id/one-words)
// //get all one-word data for an event
// router.get('/:id/wordInIteration', authorize, (req, res,next) => {
...
router.get('/:id/reflectionsOverTime', authorize, (req, res,next) => {
  ...
  function aggregateReflections(data) {
    ...
    //create two sets of data, each with one object per date and a caluclated score
    const aggregateScores = []
    const aggregateScoresWithMagnitude = []
    ...
    for (const d of dataMap) {
      ...
      const pointWithMagnitude = {
        date: d.date,
        score: averageMag
      }

      aggregateScores.push(point)
      aggregateScoresWithMagnitude.push(pointWithMagnitude)

    }
    return {aggregateScores: aggregateScores, aggregateScoresWithMagnitude: aggregateScoresWithMagnitude}
  }
})
